...
const TOPOJSON_URL = 'https://unpkg.com/topojson-client@3/dist/topojson-client.min.js';
const TERR_PROJ_URL = 'https://cdn.jsdelivr.net/npm/geo-albers-usa-territories@0.1.0/dist/geo-albers-usa-territories.js';
...
function loadLibs() {
    ...
    _libsPromise = new Promise(resolve => {
        const load = (src, cb) => {
        ...
        const afterD3 = () => {
            ...
            const needTerr = !window.geoAlbersUsaTerritories;
            if (!needTopo && !needTerr) { resolve(); return; }
            let remaining = (needTopo ? 1 : 0) + (needTerr ? 1 : 0);
            const done = () => { if (--remaining <= 0) resolve(); };
            if (needTopo) load(TOPOJSON_URL, done);
            // Loads after d3: its UMD build reads `global.d3` at load time.
            if (needTerr) load(TERR_PROJ_URL, done);
        };
        if (window.d3) { afterD3(); return; }
        load(D3_URL, afterD3);
    });
    return _libsPromise;
}
...
class NPSParksCard extends HTMLElement {
    ...
    async _init() {
        await loadLibs();
        const topoData = await d3.json(US_ATLAS_URL);
    ...
    _renderMap(topoData) {
        ...
        this._projection = window.geoAlbersUsaTerritories.geoAlbersUsaTerritories();
